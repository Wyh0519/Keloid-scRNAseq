...
# Version: PANELS-ONLY 1.2 FINAL | 2026-06-19
#
# PURPOSE
...
OUTPUT_FOLDER_NAME <- "15_PUBLICATION_PANELS_ONLY_V1_2_FIXED"
...
# 2B projection into reference score space
...
dis_proj_coords <- predict(pca_projection, newdata = s5_fib |> dplyr::select(z_Skin, z_Wound1, z_Wound7, z_Wound30))[,1:2, drop = FALSE]
dis_proj <- dplyr::bind_cols(
  s5_fib |> dplyr::select(patient_id, group), as.data.frame(dis_proj_coords)
) |> dplyr::mutate(type = "Disease cohort", id = patient_id)
write_source(ref_proj, "Figure2", "B_reference", "Reference PCA coordinates used for projection")
write_source(dis_proj, "Figure2", "B_disease", "Discovery patient coordinates projected into the frozen reference score space")
p2b <- ggplot2::ggplot() +
  ggplot2::geom_path(
    data = ref_proj, ggplot2::aes(PC1, PC2, group = donor),
    linewidth = 0.5, colour = "#A0A0A0", arrow = grid::arrow(length = grid::unit(0.07, "inches"))
  ) +
  ggplot2::geom_point(
    data = ref_proj, ggplot2::aes(PC1, PC2, colour = true_stage),
    size = 2.7, alpha = 0.85
  ) +
  ggplot2::geom_point(
    data = dis_proj, ggplot2::aes(PC1, PC2, fill = group),
    shape = 21, size = 4.0, stroke = 0.8, colour = "black"
  ) +
  ggrepel::geom_text_repel(
    data = dis_proj, ggplot2::aes(PC1, PC2, label = patient_id),
    size = 2.6, family = BASE_FONT, segment.size = 0.35, max.overlaps = Inf
  ) +
  ggplot2::scale_colour_manual(values = stage_colors, labels = stage_labels) +
  ...
  ggplot2::labs(title = "Reference projection", x = "Reference PC1", y = "Reference PC2", colour = "Reference stage", fill = NULL) +
  theme_pub() + ggplot2::theme(legend.position = "top")
...
completion_text <- c(
  ...
  "Individual panel TIFFs only; no composite figures are exported in this panels-only build",
